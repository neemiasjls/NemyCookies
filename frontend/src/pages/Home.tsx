...
export default function Home() {
  ...
  return (
    <div className="min-h-screen bg-canvas flex flex-col">
      {/* Navbar */}
      <nav className="bg-surface/85 backdrop-blur-md border-b border-line sticky top-0 z-30 shadow-card">
        <div className="max-w-6xl mx-auto px-3 py-2.5 flex items-center gap-3">
          ...
          >
            ...
            <span className="font-display font-bold text-ink text-base hidden sm:block">NemyCookies</span>
          </button>
          ...
          <div className="flex-1 relative">
            ...
            <input
              ...
              className="w-full border border-brand-line bg-surface rounded-full pl-9 pr-3 py-2 text-sm focus:outline-none focus:border-brand focus:ring-2 focus:ring-brand/10 transition-shadow"
            />
          </div>
          ...
          <button
            ...
            className="relative flex items-center gap-2 bg-brand text-brand-ink px-4 py-2 rounded-full text-sm font-semibold hover:bg-brand-strong transition-colors flex-shrink-0"
          >
            <ShoppingBag size={16} />
            {itemCount > 0 && (
              <span className="absolute -top-1.5 -right-1.5 bg-gold text-shell text-xs w-5 h-5 rounded-full flex items-center justify-center font-bold shadow">
                {itemCount}
              </span>
            )}
      ...
      <div className="relative bg-gradient-to-b from-cookie-dark via-[#43270F] to-cookie-brown text-white overflow-hidden">
        ...
        <div className="relative max-w-6xl mx-auto px-4 pt-7 pb-9 flex flex-col items-center text-center">
          ...
          <div className="flex items-center gap-2.5 mt-3">
            ...
            <Cookie size={13} className="text-gold" />
          ...
          <div className="mt-5 flex items-center gap-2.5 rounded-full pl-2 pr-4 py-2
            ...
            <span className="w-6 h-6 rounded-full bg-gold flex items-center justify-center flex-shrink-0">
              <Truck size={13} className="text-ink" />
            </span>
          ...
          <div className="flex-1 min-w-0">
            {loading ? (
              <div className="flex flex-col items-center justify-center py-16 gap-3">
                ...
                <p className="text-ink-3 text-sm">Carregando cardápio...</p>
              </div>
            ) : erro ? (
              <div className="flex flex-col items-center text-center py-16 gap-3">
                ...
                <p className="text-ink-2 font-semibold">Não conseguimos carregar o cardápio</p>
                <p className="text-ink-3 text-sm">Verifique sua conexão e tente de novo.</p>
                <button
                  onClick={carregar}
                  className="mt-1 bg-brand text-brand-ink font-bold px-6 py-2.5 rounded-full text-sm active:bg-brand-strong transition-colors"
                >
                  Tentar novamente
                </button>
                <details className="mt-3 max-w-md">
                  <summary className="text-xs text-ink-3 cursor-pointer">Detalhes técnicos</summary>
                  <p className="mt-1.5 text-[11px] text-ink-2 bg-canvas border border-line rounded-lg px-3 py-2 break-words text-left font-mono">
                    {erro}
                  </p>
                </details>
              </div>
              ...
              <div className="flex flex-col items-center text-center py-16 gap-3">
                <div className="w-14 h-14 rounded-full bg-brand-line flex items-center justify-center">
                  <Cookie size={24} className="text-brand" />
                </div>
                <p className="text-ink-2 font-semibold">Estamos sem cookies no momento</p>
                <p className="text-ink-3 text-sm">Volte em breve — tem fornada nova saindo!</p>
              </div>
            ) : filtered.length === 0 ? (
              <div className="flex flex-col items-center text-center py-16 gap-3">
                <div className="w-14 h-14 rounded-full bg-brand-line flex items-center justify-center">
                ...
                <p className="text-ink-3">Nenhum cookie encontrado para "{search}"</p>
              </div>
            ) : (
              productsByCategory
                ...
                .map(({ category, products: prods }) => (
                  <section key={category.id} className="mb-8">
                    ...
                    {category.description && (
                      <p className="text-ink-2 text-sm mb-3 leading-snug ml-3.5">{category.description}</p>
                    )}
      ...
      {itemCount > 0 && (
        <div className="fixed bottom-4 left-0 right-0 flex justify-center lg:hidden z-20 px-4">
          <button
            onClick={() => navigate('/checkout')}
            className="bg-brand text-brand-ink font-bold px-6 py-3.5 rounded-full shadow-xl flex items-center gap-3 hover:bg-brand-strong transition-colors w-full max-w-sm justify-between"
          >
            <span className="bg-surface text-brand text-xs font-bold px-2 py-0.5 rounded-full">
